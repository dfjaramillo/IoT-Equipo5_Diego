...
int lightSensorPin = A0; // Analog pin to which the LDR is connected
int ldrValue = 0; // Variable to store the light intensity value
...
void loop()
{
  ...
  //lee los datos de intensidad luminica.
  ldrValue = analogRead(lightSensorPin); // Read the analog value from A0
  ...
  if ( !isnan(h) && !isnan(t) ) {
    ...
    //Publica en el tópico de la intensidad luminica
